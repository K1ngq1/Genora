...
## 首页与工作空间入口（已确认，2026-06-23）

- `/` 作为 Genora 前端首页，不再直接承载无限画布。
- 首页 UI 参考 TapNow 风格：深色点阵背景、中心 Agent 输入框、快捷创作提示和精选推荐区域。
- 首页 Agent 输入框需要调用现有 Agnes 对话接口，完成真实对话。
- 首页需要展示当前无限画布中已接入的图片/视频模型，并复用统一模型目录，不重复硬编码模型清单。
- 首页右上角保留“工作空间”和“登录”两个入口；“工作空间”进入作品库，作品库内新建或打开项目后进入无限画布。
- “登录”按钮跳转 `/login`，用于后续接入其他分支的注册/登录前端。
- 无限画布迁移到 `/workspace` 路由，支持通过项目 ID 打开作品库项目。
